...
function collectRulesFromForm() {
  ...
  ruleItems.forEach((item) => {
    ...
    const label = item.querySelector('input[id^="label-"]').value.substring(0, 6);
...
function createRuleElement(rule, index) {
  ...
  const formattedLabel = formatLabel(rule.label);
  ...
  div.innerHTML = `
    ...
    <div class="form-row">
      ...
      <div class="preview-favicon" style="background: ${rule.backgroundColor || '#667eea'}; color: ${rule.foregroundColor || '#ffffff'};">
...
function setupRuleEventListeners(element, index) {
  ...
  const updatePreview = () => {
    const formatted = formatLabel(labelInput.value);
    const line1 = preview.querySelector('.line1');
    const line2 = preview.querySelector('.line2');
...
function importSettings(jsonData) {
  try {
    ...
    const validRules = data.rules.filter(rule => {
      ...
      label: (rule.label || '').substring(0, 6),
